...
interface IAPIClient {
  ...
  getTabGcsPrefix(dashboardName: string, tabName: string): Promise<string>;
...
class APIClient implements IAPIClient {
  ...
  // Fetch the config for a single tab and return its GCS prefix (e.g.
  // "test-platform-results/logs/<job>"), used to build per-cell links to the
  // real build results. The generated protobuf types don't cover this endpoint,
  // so we read the raw JSON (proto field names, snake_case) directly.
  async getTabGcsPrefix(
    dashboardName: string,
    tabName: string
  ): Promise<string> {
    ...
    const response = await fetch(
      `${this.baseUrl}/api/v1/dashboards/${encodedDashboard}/tabs/${encodedTab}`
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch tab config for "${dashboardName}/${tabName}": ${response.status} ${response.statusText}`
      );
    }
    const json = await response.json();
    return json?.result_source?.gcs_config?.gcs_prefix ?? '';
  }
